...
const HeroSection = () => {
  ...
  const stats = [
    { value: "12", label: t("stats.providers") },
    { value: "48", label: t("stats.gpus") },
    { value: "$0.35/hr", label: t("stats.avg_cost"), usd: 0.35 },
              ...
              {isKnown && earnings ? (
                <div className="rounded-xl border border-green-500/30 bg-green-500/10 px-6 py-4">
                  <p className="text-sm text-green-400">
                    {t("hero.detected_gpu")} <span className="font-bold text-white">{gpuDisplayName}</span> {t("hero.could_earn")}{" "}
                ...
                <div className="rounded-xl border border-border bg-card px-6 py-4">
                  <p className="text-sm text-muted-foreground mb-3">{t("hero.select_gpu")}</p>
                  <GPUManualSelector
